...
$collector = new Collector(
    new Config(getenv('DIGTRACE_SECRET') ?: null),
    new JsonlSink('/var/log/digtrace/your-app-name.production.today-yyyymmdd.jsonl'), // JSONL ファイルへ書き出し
...
$collector = new Collector(
    new Config(getenv('DIGTRACE_SECRET') ?: null),
    new JsonlSink('/var/log/digtrace/legacy-shop.production.jsonl'),
...
`statement_fingerprint` は現在の `digtrace-v1` では必須です。旧形式の JSONL（このフィールドがないもの）は新スキーマの検証対象外です。
...
php bin/digtrace export trace.jsonl > trace.export.json
php bin/digtrace export trace.jsonl --format md > trace.export.md

# 移行評価では旧系/新系をそれぞれ export して、2ファイルを AI に渡す
php bin/digtrace export legacy.jsonl > legacy.export.json
php bin/digtrace export target.jsonl > target.export.json
...
`maxDepth` / `maxShapeNodes` / `maxTimelineSize` に達した場合、digtrace の観測だけを打ち切り、アプリ本体の処理は止めません。打ち切りは `errors[]` に `capture_failure` として残ります。
