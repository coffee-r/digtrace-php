...
`examples/ci3-shop/var/flow-map.tsv` は読まないでください。
`flow_id`（8桁hex）は匿名相関IDであり仕様名ではありません。
`analysis.md` の「観測flow」列にはシナリオ名と flow_id が両方出ますが、
シナリオ名はヒントであり仕様断定の根拠ではありません。
必ず観測ログとコードで裏取りしてください。
...
`analysis.md` は `bin/tekagami analyze` が生成したMarkdownです。

- **「EC業務分岐ごとの証拠」テーブルは人手で書いた不完全な参照情報**です。
  観測から自動生成したものではありません。ここに載っていない分岐・flow・patternを積極的に探してください。
- 「customイベント」テーブルは自動集計（全イベント名 × 件数）です。
- 「観測サマリ」は自動集計です。

`tekagami.jsonl` と `report.md` は読まないでください。
...
| 確信度 | 条件 |
|--------|------|
| 高 | 観測ログ（status + signature + effects）・コード・DDL/fixture の複数種が一致、またはcustomイベントで業務分岐名が明示されている |
| 中 | 観測ログかコードの一方のみで確認。customイベントなしでSQLフローから推定している |
| 低 | 推論のみ。count=1のrare patternでテストか業務分岐か不明。run-e2e.shのペイロードのみでコード根拠がない |
...
一覧表の列: 仕様候補名 / エンドポイント / status / 確信度 / customイベント有無

各仕様候補は `### {仕様候補名}` の見出しで書き、以下の順に記述してください。

1. **エンドポイントとパターン** — `METHOD /path`、pattern id、count、status
2. **観測証拠** — signature 中の `CUSTOM:xxx`、effects の op/table、status分布
3. **実装証拠** — controller/model メソッド、関連テーブル・カラム、seed fixture
4. **仕様候補として言えること**
5. **確信度** — 高/中/低 + 1行の根拠
6. **未確定事項** — 現証拠では断定できないこと
7. **推奨アクション** — 追加E2Eシナリオまたはcustomイベント名
...
- `analysis.md` の業務分岐表はヒントとして使い、そこにない分岐も積極的に探してください。
